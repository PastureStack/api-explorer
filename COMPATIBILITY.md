...
The Server packaging contract currently consumes the `1.1.15` static archive and exposes its `ui.min.js` and `ui.min.css` files below `/api-ui/`. The `1.1.16` security candidate preserves that archive layout; changing the Server default still requires a coordinated embedded-server test.
